...
public class CompletableFuture<T> implements Future<T>, CompletionStage<T> {
    ...
    final boolean tryPushStack(Completion c) {
        ...
        // this对象的STACK值是否为h，如果是则设置为c
        return STACK.compareAndSet(this, h, c);
    }

    // 无条件得将stack设置为c,必要时重试
    ...
    // 如果RESULT的值为null，则设置为NIL对象
    /** Completes with the null value, unless already completed. */
    final boolean completeNull() {
        return RESULT.compareAndSet(this, null, NIL);
    }

    // 如果参数为空则返回NIL对象，否则返回参数自己
    /** Returns the encoding of the given non-exceptional value. */
    final Object encodeValue(T t) {
        return (t == null) ? NIL : t;
    }

    // RESULT的值为空时设置为NIL对象，不为空时设置成参数
    ...
    // RESULT结果集为null时设置成对应的异常
    /** Completes with an exceptional result, unless already completed. */
    final boolean completeThrowable(Throwable x) {
        return RESULT.compareAndSet(this, null, encodeThrowable(x));
    }

    // 1. 如果x异常不是CompletionException，则封装成CompletionException异常
    //    并通过AltResult对象返回
    // 2. 如果r对象是AltResult, 并且r的异常等于参数x，则返回参数r
    // 3. 返回AltResult对象
    /**
     * Returns the encoding of the given (non-null) exception as a
     ...
     * support parallelism.
     * 
     * 默认的线程执行器 -- ForkJoinPool.commonPool() 除非不支持并行
     * 
     */
    ...
    abstract static class Completion extends ForkJoinTask<Void>
         ...
         * 
         * 如果触发则完成对应的操作，如果存在依赖项则返回可能需要传播的依赖项
         *
         * @param mode SYNC, ASYNC, or NESTED
         */
        abstract CompletableFuture<?> tryFire(int mode);

        // 如果可能仍然存在触发，则返回true,由cleanStack调用
    ...
    abstract static class UniCompletion<T,V> extends Completion {
        ...
        final boolean claim() {
            ...
            if (compareAndSetForkJoinTaskTag((short)0, (short)1)) {
                ...
                e.execute(this); // Completion是一个Runnable,执行Completion操作
            }
            return false;
        }
    ...
    final void unipush(Completion c) {
        if (c != null) {
            while (!tryPushStack(c)) {
                if (result != null) { 如果结果集不为空
                    NEXT.set(c, null); // c对象的NEXT属性设置为Null
                    break;
                }
            }
            if (result != null)
                c.tryFire(SYNC); // 异步执行c对象的tryFire方法
        }
    }
    ...
    static final class UniApply<T,V> extends UniCompletion<T,V> {
        ...
        final CompletableFuture<V> tryFire(int mode) {
            ...
            // 如果dep, fn, src, src.result任意一个为null则返回null
        ...
        public void run() {
            ...
            if ((d = dep) != null && (f = fn) != null) {
                ...
                if (d.result == null) {
                    try {
                        f.run(); //执行具体的任务
                        d.completeNull(); //设置结果集为null
                    } catch (Throwable ex) {
                        d.completeThrowable(ex);
                    }
                }
                d.postComplete(); //执行后续的依赖任务
            }
        }
    }
     ...
     * 
     * 任务在线程池异步得执行完，执行完指定的操作后会返回一个新的CompletableFuture对象
     *
     * @param runnable the action to run before completing the
     * returned CompletableFuture
     * @return the new CompletableFuture
     */AsyncRun extends ForkJoinTask
