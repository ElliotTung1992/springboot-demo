...
public class OracleSourceConnectorUtils{
    ...
    protected void parseTableWhiteList(){
        ...
        //新增非空判断
        if(tableWhiteList != null && !tableWhiteList.equals("")){
...
//          }
    ...
    protected Map<String,LinkedHashMap<String,String>> createDataSchema2(String owner,String tableName,String sqlRedo,String operation) throws Exception{
        Map<String,LinkedHashMap<String,String>> allDataMap = null;
        if (config.getParseDmlData()){
            if (!tableSchema.containsKey(owner+DOT+tableName)){
                loadTable(owner, tableName,operation);
            }
            allDataMap = parseSql(owner, tableName, sqlRedo);
        }
        return allDataMap;
    }
