...
//      }
      ...
      logMinerStartStmt.setLong(1, 5444643906l);
      logMinerStartStmt.setLong(2, 5444643907l);
...
//      logMinerSelect.setFetchSize(config.getDbFetchSize());
      ...
      //设置事务编号
      logMinerSelect.setString(1, "0400110031C70100");
  ...
  public List<SourceRecord> poll() throws InterruptedException {
    ...
    try {
      ...
      while(!this.closed && logMinerData.next()){
        ...
        if (ix % 100 == 0) log.info(String.format("Fetched %s rows from database %s ",ix,config.getDbNameAlias())+" "+row.getDate());
  ...
  private Struct setValueV2(Data row,DataSchemaStruct dataSchemaStruct) {    
    Struct valueStruct = new Struct(dataSchemaStruct.getDmlRowSchema())
              ...
              .put(TIMESTAMP_FIELD, row.getDate())
